...
class FeedbackRepositoryImpl : FeedbackRepository {
    ...
    override suspend fun getFeedbackMessages(userId: String, withUserId: String): Either<FirestoreError, List<FeedbackMessage>> {
        return try {
            val snapshot = db.collection("feedback")
                .whereIn("senderId", listOf(userId, withUserId))
                .whereIn("receiverId", listOf(userId, withUserId))
                .orderBy("timestamp")
                .get()
                .await()
